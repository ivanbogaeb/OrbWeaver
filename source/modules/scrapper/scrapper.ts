...
export const scrapper = async (requestURL:string, images:boolean, extractURLs:boolean, verbose: boolean) => {
    ...
    let articles:any = await request(requestURL, cheerio, extractURLs);
    verbose && console.log(`[+] - News articles gathered...`);
    if (articles){
        if (images || extractURLs){
        for (let i = 0; i < articles.length; i++){
            await timer(3000).then(async () => { // 3 seconds is a good time to wait
